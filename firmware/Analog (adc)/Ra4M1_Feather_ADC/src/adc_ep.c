...
fsp_err_t adc_read_data(void)
{
    ...
    err = R_ADC_Read (&g_adc_ctrl, ADC_CHANNEL_0, &g_adc_data1);
    err = R_ADC_Read (&g_adc_ctrl, ADC_CHANNEL_1, &g_adc_data2);
    err = R_ADC_Read (&g_adc_ctrl, ADC_CHANNEL_2, &g_adc_data3);
    err = R_ADC_Read (&g_adc_ctrl, ADC_CHANNEL_3, &g_adc_data4);
    err = R_ADC_Read (&g_adc_ctrl, ADC_CHANNEL_4, &g_adc_data5);
    /* handle error */
    if (FSP_SUCCESS != err)
    {
        /* ADC Failure message */
